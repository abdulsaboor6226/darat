...
class Elementinvader_index extends Winter_MVC_Controller {
            ...
            if (is_dir($path))
            {
                ...
                while (($file = readdir($dh)) !== false)
                {
                    if(file_exists($path.$file.'/description.xml') || file_exists($path.$file.'/description.pac'))
                    {
                        ...
                        foreach($required_plugins->plugin as $key=>$plugin)
                        {
	...
	{       
        ...
        if(strpos($template, 'download_kit_') === 0)
        {
            ...
            if(count($plugins_to_activate) == 0)
            {
                ...
                else if(!empty($ret_call_obj->purchase_message))
                {
                    ...
                    $results['page_url'] = admin_url('admin.php?page=elementinvader_contact&template='.$template.'&page_title='.$page_title);
        ...
        else
        {
            ...
            {        
                ...
                $results['message'] = __('Import failed, json error, please report issue here','elementinvader');
            }
    ...
    public function generate_zip($elementor_post_id = NULL, $data = array())
    {
        ...
        if(isset($data['screenshoot'])) {
            $screenshot_url = get_attached_file($data['screenshoot'], true);
        }

        if(isset($data['screenshoot_large'])) {
            $screenshot_url_large = get_attached_file($data['screenshoot_large'], true);
        }

        if(empty($screenshot_url) && empty($screenshot_url_large) && $post_data->post_type == "envato_tk_templates" || TRUE) {
            $attachment_id = get_post_thumbnail_id( $elementor_post_id );
            if($attachment_id) {
                $screenshot_url_large = $screenshot_url = get_attached_file($attachment_id, true);
            }
            $elementor_post_id = $this->input->post_get('post');
        }
