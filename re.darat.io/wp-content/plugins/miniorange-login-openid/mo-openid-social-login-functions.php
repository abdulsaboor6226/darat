...
function mo_openid_process_user_details( $appuserdetails, $appname ) {
	...
	if ( get_option( 'mo_openid_auto_register_enable' ) ) {
		...
		if ( get_option( 'mo_openid_account_linking_enable' ) ) {
			echo wp_kses( mo_openid_account_linking_form( $user_name, $user_email, $first_name, $last_name, $user_full_name, $user_url, $user_picture, $appname, $social_user_id ), $allowed_html );
			exit;
		} elseif ( get_option( 'mo_openid_enable_profile_completion' ) && ( $session_values['user_email'] == '' || $session_values['username'] == '' ) ) { // if newa user and profile completion is enabled
