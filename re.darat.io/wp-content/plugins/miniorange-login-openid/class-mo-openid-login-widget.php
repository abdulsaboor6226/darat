...
function mo_openid_login_validate() {
	...
	if ( isset( $_REQUEST['option'] ) and strpos( sanitize_text_field($_REQUEST['option']), 'getmosociallogin' ) !== false ) { 	// phpcs:ignore 
		...
		mo_pop_show_verify_password_page();
	} elseif ( isset( $_POST['mo_openid_account_linking_nonce'] ) and isset( $_POST['option'] ) and strpos( sanitize_text_field( $_POST['option'] ), 'mo_openid_account_linking' ) !== false ) {
		...
		if ( ! wp_verify_nonce( $nonce, 'mo-openid-account-linking-nonce' ) ) {
			...
			if ( ! isset( $_POST['mo_openid_create_new_account'] ) ) {
				...
				if ( get_option( 'mo_openid_enable_profile_completion' ) && ( $user_details['user_email'] == '' || $user_details['username'] == '' ) ) { // if newa user and profile completion is enabled
					echo esc_attr( mo_openid_profile_completion_form( $user_details ) );
					exit;
				} else {
