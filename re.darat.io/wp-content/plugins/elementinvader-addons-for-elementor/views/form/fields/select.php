...
if($element['show_label'])
    $output .='<label for="'.$field_id.'">'.$element['field_label'].$required_icon.'</label>';
            $output .='<select '.$helper_attr.' name="'.$element['field_label'].'" id="'.$field_id.'" type="select" class="elementinvader_addons_for_elementor_f_field" '.$required.' value="'.$value.'" placeholder="'.$element['placeholder'].'" >
                %1$s
            </select>
        </div>';
