...
if($element['show_label'])
    $output .='<label for="'.esc_attr($field_id).'">'.esc_html($element['field_label']).esc_html($required_icon).'</label>';
            ...
            $output .='    
                name="'.esc_attr($element['field_label']).'" id="'.esc_attr($field_id).'" type="'.esc_attr($field_tyle).'" class="elementinvader_addons_for_elementor_f_field" '.wp_kses_post($required).' value="'.esc_attr($value).'" placeholder="'.esc_attr($element['placeholder']).'" >
            </div>';
}
