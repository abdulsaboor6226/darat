...
class Ajax_Handler {
	...
	public function ajax_send_form() {
            ...
            if(!isset($post['element_id']) || empty($post['element_id'])){
                $ajax_output['code'] = $this->INVALID_FORM;
            ...
            if(isset($post['g-recaptcha-response']))
                if($this->valid_recaptcha($post['g-recaptcha-response'], $form_data['recaptcha_secret_key']) === TRUE)
                {
                    /* success */
                }
                else
            ...
            if(!isset($form_data['disable_mail_send']) || empty($form_data['disable_mail_send'])) {
                ...
                $message = '
                    <html>
                    <head>
                      <title>'.$this->_ch($form_data['mail_data_subject']).'</title>
                    </head>
                    <body>
                        {dynamic_values}                            
                    </body>
                ...
                $subject = $this->_ch($form_data['mail_data_subject']).' '.esc_html($custom_message);
                $ret = false;
            ...
            if(!isset($form_data['disable_mail_send']) || empty($form_data['disable_mail_send'])){
                if($email) {
                    ...
                    $message = '
                        ...
                        <body>
                            <h2>'.esc_html__( 'Details from your message.', 'elementinvader-addons-for-elementor' ).'</h2>
                            {dynamic_values}                            
                        </body>
                        </html>
                    ';
            ...
            if($ret){
                ...
                $ajax_output['code'] = $this->SERVER_ERROR;
