...
if(!class_exists('ct_Tabs')) {
  class ct_Tabs extends WP_Widget {
     ...
     function widget($args, $instance) {
         ...
         $latest = isset( $instance['latest'] ) ? esc_html( $instance['latest'] ) : '';
         $comments = isset( $instance['comments'] ) ? esc_html( $instance['comments'] ) : '';
         $tags = isset( $instance['tags'] ) ? esc_html( $instance['tags'] ) : '';
     ...
     function form($instance) {
       ...
       $tags = isset( $instance['tags'] ) ? esc_attr( $instance['tags'] ) : '';
         ...
         ?>
         <p>
           <label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php _e( 'Number of posts:', 'contempo' ); ?>
          ...
          <p>
              <label for="<?php echo esc_attr($this->get_field_id( 'order' )); ?>"><?php _e( 'First Visible Tab:', 'contempo' ); ?></label>
       ...
       <p>
           <input id="<?php echo esc_attr($this->get_field_id( 'latest' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'latest' )); ?>" type="checkbox" <?php if($latest == 'on') echo 'checked="checked"'; ?>><?php _e( 'Latest', 'contempo' ); ?></input>
       </p>
       <p>
           <input id="<?php echo esc_attr($this->get_field_id( 'comments' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'comments' )); ?>" type="checkbox" <?php if($comments == 'on') echo 'checked="checked"'; ?>><?php _e( 'Comments', 'contempo' ); ?></input>
       </p>
       <p>
           <input id="<?php echo esc_attr($this->get_field_id( 'tags' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'tags' )); ?>" type="checkbox" <?php if($tags == 'on') echo 'checked="checked"'; ?>><?php _e( 'Tags', 'contempo' ); ?></input>
         </p>
         <?php
     }
  ...
  function wpb_load_ct_Tabs() {
      register_widget( 'ct_Tabs' );
  }
  add_action( 'widgets_init', 'wpb_load_ct_Tabs' );
}
...
if(!function_exists('ct_widget_load_tabs_js')) {
  function ct_widget_load_tabs_js(){
    ?>
      <script>
...
add_action( 'wp_footer', 'ct_widget_load_tabs_js' );
  ...
  <li>
      <div class="pad10">
          ...
          <div class="col <?php if(has_post_thumbnail()) { echo 'span_9'; } else { echo 'span_12'; } ?>">
              <h5 class="marB0"><a title="<?php the_title(); ?>" href="<?php the_permalink() ?>"><?php the_title(); ?></a></h5>
