...
class Wdk_listing extends Winter_MVC_Controller {
	...
	public function index()
	{
        ...
        if($this->form->run($rules))
        {
            ...
            $listing_data_fields = array('category_id', 'location_id', 'address', 'lat', 'lng', 'listing_images','listing_plans_documents', 'is_featured', 'is_activated','is_approved', 'package_id','rank','subscription_id','user_id_editor');
                ...
                foreach ($image_ids as $image_id) {
                    if(is_numeric($image_id))
                    {
                        ...
                        if($image_path) {
                            ...
                            if(strlen($listing_data['listing_images_path'].$next_path)<195) {
                        ...
                        if($image_url) {
                            ...
                            /* check length listing_images_path + next image + comma, should be less 200*/
                            if(strlen($listing_data['listing_images_path_medium'].$next_path)<240) {
            ...
            if(isset($data['listing_sub_categories']) && is_array($data['listing_sub_categories'])) {
            ...
            if(isset($data['listing_sub_locations']) && is_array($data['listing_sub_locations'])) {
