...
class Location_m extends Winter_MVC_Model {
    ...
    public function get_pagination($limit, $offset, $where = array(), $order_by = NULL)
    {
        ...
        $this->db->select($this->_table_name.'.*, COUNT('.$this->listing_m->_table_name.'.post_id) AS listings_counter');
        $this->db->from($this->_table_name);

        $this->db->join($this->listing_m->_table_name.' ON '.$this->listing_m->_table_name.'.location_id = '.$this->_table_name.'.idlocation', TRUE, 'LEFT');
