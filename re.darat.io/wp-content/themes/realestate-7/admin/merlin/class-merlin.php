...
class Merlin {
	...
	protected function welcome() {
		...
		$ct_max_execute = ini_get('max_execution_time');
		$ct_php_memory_limit = ini_get('memory_limit');
		$ct_php_memory_limit = str_replace('M', '', $ct_php_memory_limit);
					    
	    $sqlversion = $wpdb->get_var("SELECT VERSION() AS version");

		### Function: Format Bytes Into TiB/GiB/MiB/KiB/Bytes
		if(!function_exists('ct_format_filesize')) {
		    function ct_format_filesize($rawSize) {
		...
		function ct_format_php_size($size) {
		    if (!is_numeric($size)) {
		...
		if(!function_exists('ct_get_php_max_execution')) {
		    function ct_get_php_max_execution() {
		        ...
		        if($max_execute < 9999) {
			        return '<span class="below-req">' . $max_execute . 's</span>';
			    } else {
			    	return '<span class="meets-req">' . $max_execute . 's</span>';
			    }
		    }
		}
		...
		if(!function_exists('ct_get_php_memory_limit')) {
		    function ct_get_php_memory_limit() {
		        ...
		        if($memory_limit < 512) {
			        return '<span class="below-req">' . $memory_limit . 'M</span>';
			    } else {
			    	return '<span class="meets-req">' . $memory_limit . 'M</span>';
			    }
		    }
		}
		...
		<div class="merlin__content--transition">
			...
			<p style="margin-bottom: 20px;"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>

			<style>
				#server-health-info { background: #f8f9fd; margin: 30px 0 0 0; padding: 20px; border-radius: 3px;}
					#server-health-info h3 { display: inline-block; margin: 0 0 12px 0; font-weight: 300;}
						...
						.server-info li { clear: both;}
							.server-feature { float: left;}
							.server-value { float: right;}
			</style>

			<?php
				echo '<div id="server-health-info">';
					echo '<h3>Server Health</h3>';
					
					if($ct_max_execute >= 9999) {
						echo '<span id="health-meter" class="meets-requirements"></span>';
						echo '<p id="check-req"><strong>Meets Requirements</strong>, setup & import functions will operate smoothly.</p>';
					} elseif ($ct_max_execute < 9999) {
						echo '<span id="health-meter" class="does-not-meet-requirements"></span>';
						echo '<p id="check-req"><strong>Does not meet requirements</strong><br />';
						echo 'You may see timeout issues resulting in a broken demo import. Before you proceed please check the <a href="https://contempothemes.com/wp-real-estate-7/docs/installation-demo-import/#theme-requirements" target="_blank"><em>theme requirements</em></a>.</p>';
					}

					echo '<ul class="server-info">';
						echo '<li><span class="server-feature">PHP Memory Limit</span> <span class="server-value">' . ct_get_php_memory_limit() . '</span></li>';
						echo '<li><span class="server-feature">PHP Max Execution Time</span> <span class="server-value">' . ct_get_php_max_execution() . '</span></li>';
	...
	protected function license() {
		...
		<div class="merlin__content--transition">
			...
			<p id="license-text" style="margin-bottom: 10px;"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>
			<a class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external" href="https://contempothemes.com/docs/installation-setup-wizard/#2-toc-title" target="_blank">Where can I find my purchase code?</a>
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				<p style="margin-top: 8px;"><?php esc_html_e( 'Select a prebuilt site:', 'merlin-wp' ); ?></p>
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			* Description: This is a child theme of {$parent}, generated by Real Estate 7.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
